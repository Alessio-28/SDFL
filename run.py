...
def main() -> None:
    ...
    _ = parser.add_argument("--log", const = "sdfl.log", nargs = "?", type = str,  help = "Enables logging. Log file name can be passed to this argument. Default: sdfl.log.")
    help : str = (
        "Parameters must be witten in the following order: theta gamma c eta epsilon.\n"
        "Parameters can also be written in parameters.json.\n"
...
def export_parameters(parameters : Parameters) -> None:
    ...
    with open("./parameters.json", "w") as p:
        json.dump(param, p, indent = 4, separators = (",", ": "))

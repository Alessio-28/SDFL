...
def set_parser_utils_group(parser: ap.ArgumentParser) -> None:
    parser.add_argument(
        ...
        "--list-problems",
        action="store_true",
        help="Prints available problems.",
    )
    parser.add_argument(
        "-j",
        "--create-json",
        action="store_true",
        help=f"Creates default {data_json.DATA_JSON}",
    )
    parser.add_argument(
        "-t",
        "--create-template-problem",
        action="store_true",
        help=f"Creates default {problem_from_file.PROBLEM_PY}",
    )
...
def set_parser_run_group(parser: ap.ArgumentParser) -> None:
    ...
    problem_group.add_argument(
        "--from-file",
        action="store_true",
        help=f"A problem can be defined in {problem_from_file.PROBLEM_PY} file and then tested by the algorithm.",
    )
    ...
    run_group.add_argument(
        ...
        type=int,
        dest="MAX",
        help="Maximum number of function evaluations before SDFL terminates.",
    )
    run_group.add_argument(
        "--min-step",
        nargs=1,
        type=np.float64,
        dest="MIN",
        help="Minimum step value before SDFL terminates.",
    )
    ...
    run_group.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Print intermediate results of the algorithm.",
    )

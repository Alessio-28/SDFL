...
name      : str = "l1hilb(30)"
startp    : npt.NDArray[np.float64] = np.ones(30, dtype = np.float64)
...
def feval(x : npt.NDArray[np.float64]) -> np.float64:
    i : npt.NDArray[np.float64] = np.arange(1, 31, dtype = np.float64)
    j : npt.NDArray[np.float64] = np.arange(1, 31, dtype = np.float64)
    I = np.tile(i, (30, 1))
